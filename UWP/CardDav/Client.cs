    ...
    public class Client
    {
        ...
        private Dictionary<string, string> Query(string url, string method, string content = null, string contentType = null)
        {
            ...
            if (!string.IsNullOrEmpty(authentication) || !string.IsNullOrEmpty(oAuth2AccessToken))
            {
                ...
                var myRequest = new HttpRequestMessage(new HttpMethod(method), url);

                if (!string.IsNullOrEmpty(authentication))
                {
                    handler.Credentials = new NetworkCredential(this.username, this.password);
                }
                else if (!string.IsNullOrEmpty(oAuth2AccessToken))
                {
                    ...
                    myRequest.Headers.Add(authorizationHeaderName, string.Format("{0} {1}", oAuth2TokenType, oAuth2AccessToken));
                }

                var client = new HttpClient(handler);
                client.DefaultRequestHeaders.Add("User-Agent", UserAgent + Version);
                
                if (content != null)
                {
                    if (contentType != null)
                    {
                        myRequest.Content = new StringContent(content, Encoding.UTF8, contentType);
                    }
                    else
                    {
                        myRequest.Content = new StringContent(content, Encoding.UTF8);
                    }
                }

                var response = client.SendAsync(myRequest);
                response.Wait();
                var message = response.Result;
                queryResponse.Add("status", message.StatusCode.ToString());
                var responseContent = message.Content.ReadAsStringAsync();
                responseContent.Wait();
                queryResponse.Add("response", responseContent.Result);
