...
namespace CardDav.Utils.MIME
{
    ...
    public class MimeReader
    {
            ...
            */
            ...
            while(true){
                ...
                if(match.Success && match.Index == m_Offset){
                    ...
                    try{
                        if(string.Equals(match.Groups["encoding"].Value,"Q",StringComparison.OrdinalIgnoreCase)){
                            retVal.Append(Mimes.QDecode(Encoding.GetEncoding(match.Groups["charset"].Value),match.Groups["value"].Value));
                        }
                        else if(string.Equals(match.Groups["encoding"].Value,"B",StringComparison.OrdinalIgnoreCase)){
                            retVal.Append(Encoding.GetEncoding(match.Groups["charset"].Value).GetString(Net.FromBase64(Encoding.UTF8.GetBytes(match.Groups["value"].Value))));
                        }
            ...
            */
            ...
            if(peek == -1){
                ...
                word = encodedword_regex.Replace(word,delegate(Match m){
                    ...
                    try{
                        if(string.Equals(m.Groups["encoding"].Value,"Q",StringComparison.OrdinalIgnoreCase)){
                            return Mimes.QDecode(Encoding.GetEncoding(m.Groups["charset"].Value),m.Groups["value"].Value);
                        }
                        else if(string.Equals(m.Groups["encoding"].Value,"B",StringComparison.OrdinalIgnoreCase)){
                            return Encoding.GetEncoding(m.Groups["charset"].Value).GetString(Net.FromBase64(Encoding.UTF8.GetBytes(m.Groups["value"].Value)));
                        }
                        // Failed to parse encoded-word, leave it as is. RFC 2047 6.3.
                        else{
                            return encodedWord;
        ...
        public bool StartsWith(string value)
        {
            ...
            return m_Source.Substring(m_Offset).StartsWith(value,StringComparison.OrdinalIgnoreCase);
        }
